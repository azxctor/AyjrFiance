...
import com.hengxin.platform.common.dto.annotation.DomainField;
...
public abstract class ExceptionHandler {
    ...
    protected void setFieldErrorMap(final String fieldName, final String beanName, Class<?> rootClass,
            final Class<?> dtoClass, List<ValidationResultDto> errorData, final String message, final String formId, boolean notManually) {
    	LOGGER.debug("setFieldErrorMap() invoked, field {} ", fieldName);
        boolean isExist = false;
        final String errorField = getErrorField(fieldName, rootClass, dtoClass, notManually);
     ...
     * @return
     */
    private String getErrorField(String fieldName, Class<?> rootClass, Class<?> dtoClass, boolean notManually) {
        ...
        if (pathInfo != null && pathInfo.length > 0) {
            for (String parseFieldName : pathInfo) {
                String arrayIndex = "";
     ...
     */
    private Class<?> getNextRootClass(Class<?> rootClass, final String parseFieldName, final boolean isFound)
     ...
     * @return
     */
    private Field getFieldForAll(Class<?> rootClass, String fieldName) {
    ...
    private String getJsonName(final Field errorField, Class<?> dtoClass, boolean notManually) {
        ...
        if (dtoFields != null && dtoFields.length > 0) {
            for (Field field : dtoFields) {
                ...
                String fieldName = (domainField != null) ? domainField.field() : field.getName();
