...
public class ShiroJdbcRealm extends JdbcRealm {
    ...
    /**
     * The query used to retrieve the last login failure time. The returned timestamp should follow the format {@value #LAST_LOGIN_FAIL_DATE_FORMAT}}
     */
    ...
    */
    ...
    protected void assertCredentialsMatch(AuthenticationToken token, AuthenticationInfo info)
    	...
    	if (token instanceof PassThroughAuthenticationToken) {
    		return;
    	}
        ...
        try {
            ...
            LOGGER.debug("assertCredentialsMatch completed for user {}, total time spent: {}ms", token.getPrincipal(), end-start);
        } catch (IncorrectCredentialsException ice) {
            executeUpdate(loginFailureQuery, new String[] { token.getPrincipal().toString() });
            throw ice;
        } catch (LegacyPasswordMatchException ex) {
            executeUpdate(passwordMigrateQuery,
                    new String[] { passwordService.encryptPassword(token.getCredentials()),
    ...
    protected boolean hasLockTimePassed(String userName){
        ...
        if(result.length==0 ||result[0]==null){//it is supposed to only happen when the column is null for a dirty record
            return true;
        }
        Date date = null;
        try {
    ...
    */
     ...
     * @param passwordMigrateQuery Set passwordMigrateQuery value
     */
    ...
    /**
     * @param passwordService Set passwordService value
     */
