package com.hengxin.platform.security.util;
...
public class KaptchaSupport extends KaptchaExtend{
    ...
    private Logger LOG = LoggerFactory.getLogger(KaptchaSupport.class);
    ...
    public boolean validateCaptcha(String captcha, HttpSession session, HttpServletRequest request){
		String currentCaptcha = (String) session.getAttribute(sessionKeyValue);
		if (currentCaptcha == null) {
			LOG.warn("No captcha found in session: {}, session id: {}, request cookie: {}",
					captcha, session.getId(), request.getHeader("Cookie"));
		}
