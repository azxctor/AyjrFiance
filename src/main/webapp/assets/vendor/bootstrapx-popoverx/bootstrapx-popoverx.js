...
!function($) {
  ...
  Popoverx.prototype = $.extend({}, $.fn.popover.Constructor.prototype, {
    ...
    , clickery: function(e) {
      ...
      // set popover's dim's
      this.options.width  && this.tip().find('.popover-inner,.popover-content').width(  this.options.width  );
      this.options.height && this.tip().find('.popover-inner,.popover-content').height( this.options.height );
    ...
    , show: function () {
      ...
      if (this.hasContent() && this.enabled) {
        ...
        $tip
          ...
          .appendTo(inside ? this.$element : this.options.elementToAppend)
        ...
        if(this.options.ensure_visiable){
          ...
          var bp = { w: this.options.elementToAppend.outerWidth(), h:this.options.elementToAppend.outerHeight()};
	      switch (inside ? placement.split(' ')[1] : placement) {
	        case 'bottom':
        ...
        tp.top=tp.top-this.options.elementToAppend.offset().top+this.options.elementToAppend[0].scrollTop;
        tp.left=tp.left-this.options.elementToAppend.offset().left
        $tip
          .css(tp)
          .addClass(placement)
          .addClass('in')
      ...
      if (this.hasContent() && this.enabled) {
        ...
        switch (inside ? placement.split(' ')[1] : placement) {
          ...
          case 'top':
            ...
            break
          case 'left':
            tp = {top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left + pos.width - actualWidth - elementWidth}
            break
          case 'right':
            tp = {top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left + pos.width}
            break
