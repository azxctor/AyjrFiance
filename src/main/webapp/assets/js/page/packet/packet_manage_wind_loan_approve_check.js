...
require(['packet_manage','global'],
    function(packet,global){
        ...
        var returnOperBtn = function(data,type,full){
        	...
        	if(full.canLoanApproveConfirm){
                btnHtml += packet.renderTableBtn("放款","btn-loan","fa icon-coin",full.id);
		...
		var wlac_apple_All=function(){ 
			...
			if (ids && ids.length > 0){
				...
				var urlStr=global.context+"/web/financingpackage/batchLoanapproveconfirm";
        ...
        var init = function(){
            ...
            options.sAjaxSource = global.context+"/web/financingpackage/approveconfirm/getpackagelist";
        ...
        var bindEvent = function(){
            ...
            $("#table-wrapper").on("click",".btn-loan",function(){
                 ...
                 packet.getDetails(packageId,"loanapproveconfirm");
                 packet.showDetails();
            })//融资包详情
    		...
    		$('#wlac-apple-All').on('click', function(){
    			...
    			if (ids.length<1){
    				...
    				$("#content_manage_wind_loan").html('至少选择一条放款审批复核条目 !');
    				$("#btn_manage_wind_loan").hide();
    			}else{ 
    				$("#title_manage_wind_loan").html('确认提示');  
    				$("#content_manage_wind_loan").html('是否对以下'+ids.length+'个融资包:<br/>'+contn+'进行批量放款操作？');
    				$("#btn_manage_wind_loan").show();
    			}
    			
    			$("#modal-approve_manage_wind_loan").modal();
    		});
